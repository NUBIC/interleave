...
RSpec.describe Concept, type: :model do
  ...
  it 'reports standard concepts', focus: false do
    expect(Concept.standard).to match_array([@concept_condition_type, @concept_procedure_type])
    @concept_condition_type.standard_concept = nil
    @concept_condition_type.save
    expect(Concept.standard).to match_array([@concept_procedure_type])
  end

  it 'reports valid concepts', focus: false do
    expect(Concept.valid).to match_array([@concept_condition_type, @concept_procedure_type, @concept_domain])
    @concept_condition_type.invalid_reason = 'U'
    @concept_condition_type.save!
    expect(Concept.valid).to match_array([@concept_procedure_type, @concept_domain])
  end
end
