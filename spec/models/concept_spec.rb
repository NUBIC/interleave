...
RSpec.describe Concept, type: :model do
  before(:each) do
    ...
    @concept_procedure_type = FactoryGirl.create(:concept_procedure_type, concept_id: 5, concept_name: 'little my')
    @concept_route = FactoryGirl.create(:concept_route, concept_id: 6, concept_name: 'lucy')
    @concept_unit = FactoryGirl.create(:concept_unit, concept_id: 6, concept_name: 'groke')
  end

  it 'reports standard concepts', focus: false do
    expect(Concept.standard).to match_array([@concept_condition_type, @concept_drug_type, @concept_measurement_type, @concept_procedure_type, @concept_route, @concept_unit])
    @concept_condition_type.standard_concept = nil
    @concept_condition_type.save
    expect(Concept.standard).to match_array([@concept_drug_type, @concept_measurement_type, @concept_procedure_type, @concept_route, @concept_unit])
  end

  it 'reports valid concepts', focus: false do
    expect(Concept.valid).to match_array([@concept_condition_type, @concept_domain, @concept_drug_type, @concept_measurement_type, @concept_procedure_type, @concept_route, @concept_unit])
    @concept_condition_type.invalid_reason = 'U'
    @concept_condition_type.save!
    expect(Concept.valid).to match_array([@concept_domain, @concept_drug_type, @concept_measurement_type, @concept_procedure_type, @concept_route, @concept_unit])
  end
