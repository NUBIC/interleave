class ConditionOccurrence < ActiveRecord::Base
  ...
  scope :by_interleave_data_point, -> (interleave_data_point_id) { joins("JOIN interleave_entities ON condition_occurrence.condition_occurrence_id = interleave_entities.fact_id AND interleave_entities.cdm_table = 'condition_occurence'").where('interleave_entities.interleave_datapoint_id = ?', interleave_data_point_id) }
