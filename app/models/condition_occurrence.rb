class ConditionOccurrence < ActiveRecord::Base
  ...
  validates_presence_of :condition_concept_id, :condition_start_date, :condition_type_concept_id, :interleave_datapoint_id, :interleave_registry_cdm_source_id
  ...
  def self.by_interleave_data_point(interleave_data_point_id)
    joins("JOIN interleave_entities ON condition_occurrence.condition_occurrence_id = interleave_entities.fact_id AND interleave_entities.cdm_table = 'condition_occurrence'").where('interleave_entities.interleave_datapoint_id = ?', interleave_data_point_id)
  end
